...
> [!NOTE]
> 本文档为设计规格。当前实际实现状态：
> - **IP 过滤中间件** (`security/middleware.ts`) 已存在但未集成到主 Elysia 应用中
> - **PIN 认证** 逻辑读取 `X-MSP-PIN` header 但未在现有端点上强制执行
> - **路径沙箱** 已在 `media.ts` 路由中实现（通过 `path.join` + `startsWith` 校验）
> - **速率限制 / JWT** 尚未实现
> - 所有现有 API 端点当前均为开放访问（无鉴权）
